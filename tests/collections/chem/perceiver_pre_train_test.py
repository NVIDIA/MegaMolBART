...
@pytest.fixture
def tokenizer():
    # cfg = MolEncTokenizerFromSmilesConfig(TEST_TOKENIZER_CONFIG)
    cfg = MolEncTokenizerFromSmilesConfig({'smiles': react_data + prod_data})
    _tokenizer = MolEncTokenizer.from_smiles(cfg.smiles["smiles"], cfg.regex, mask_scheme="replace")
    return _tokenizer
...
def test_bart_decode_shape(model, sampler, tokenizer, args):
    ...
    if args.encoder_type == "perceiver":
        react_mask = torch.zeros((memory.shape[0:2]), dtype=react_mask.dtype, device=react_mask.device)
    batch_input = {
        "decoder_input": prod_ids.cuda(),
        "decoder_pad_mask": prod_mask.cuda(),
